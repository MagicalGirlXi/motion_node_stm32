...
#define get_ms      get_ms  //应该用不到 定义一个空的函数
//static inline int reg_int_cb(struct int_param_s *int_param)
//{
//    return msp430_reg_int_cb(int_param->cb, int_param->pin, int_param->lp_exit,
//        int_param->active_low);
//}
#define log_i     printf // do {} while (0)   log_i(...)
#define log_e     printf //do {} while (0)    log_e(...)
//修改区域完毕————————————————————————————————————————————	
...
//    unsigned char lp_accel_odr;  //如果不注释 会报错 提示不支持的设备————————————————————————————————————————————————————————
...
const struct gyro_reg_s reg = {
    ...
    .mem_start_addr = 0x6E,
    .prgm_start_h   = 0x70
		/*  ____________________________________________________________________________________ 可能需要替换上面的定义写法
			0x75,
			0x19,
			0x1A,
			0x0C,
...
const struct hw_s hw = {  //定义方法可能和上面一样 因为这样定义结构体是非GUN标准  
    .addr           = 0x68,
    .max_fifo       = 1024,
    .num_reg        = 118,
    .temp_sens      = 340,
 ...
 *  @return     0 if successful.
 */
int mpu_init(void) //修改为 不需要参数struct int_param_s *int_param  相应头文件声明 也要改
{
